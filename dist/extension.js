...
function extractTestCaseIds(editor, line) {
  ...
  if (!testCaseIds) {
    ...
    for (let i = line - 2; i >= startLine; i--) {
      ...
      const match2 = text.match(/(#\s*)?ADO_IDs:\s*((?:TC_\d+,?\s*)+)/);
      if (match2) {
...
function showErrorInStatusBar(message, duration = 5e3) {
  ...
  statusBarItem.color = new vscode2.ThemeColor("errorForeground");
  statusBarItem.show();
